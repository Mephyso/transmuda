...
public class US8_StepDefs {
    ...
    public void verify_the_repeat_number_is(Integer int1) {
        ...
        Assert.assertEquals("1",createEventPage.inputInterval.getAttribute("value"));
    }

    @When("Clear\\(delete) the number {int}")
    public void clear_delete_the_number(Integer int1) {
        BrowserUtils.sleep(3);
        createEventPage.inputInterval2.clear();
    }

    @Then("Verify the app displays “This value should not be blank")
    public void verify_the_app_displays_this_value_should_not_be_blank() {
        BrowserUtils.sleep(3);
        createEventPage.spanThisValueShouldNot3.getText();
        Assert.assertEquals(createEventPage.spanThisValueShouldNot3.getText(),"This value should not be blank.");
    }
